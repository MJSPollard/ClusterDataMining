...
public class PSO
{
	// many parameters for this class, having large functions that need to be
	// updated
	...
	public void runPSO(double data[][], int swarmNum, int clusterNum)
	{
		...
		for (int j = 0; j < numPart; j++)
		{
			...
			{
				R[j][l] = xMin + (xMax - xMin) * rand.nextDouble(); // starts
																	// each
																	// particle
																	// at random
																	// location
				V[j][l] = vMin + (vMax - vMin) * rand.nextDouble(); // sets
																	// particles
																	// velocity
			}
		}
		...
		for (int j = 0; j < numT; j++)
		{
			...
			for (int p = 0; p < numPart; p++)
			{
				...
				if (M[p] > pBestValue[p])
				{
					...
					for (int l = 0; l < numD; l++)
					{
						pBestPosition[p][l] = R[p][l]; // finds local best
														// location
					}
				}
			...
			for (int p = 0; p < numPart; p++)
			{
				...
				for (int l = 0; l < numD; l++)
				{
					...
					r1 = rand.nextDouble();
					r2 = rand.nextDouble();
					V[p][l] = chi * w * (V[p][l] + r1 * c1 * (pBestPosition[p][l] - R[p][l])
							+ r2 * c2 * (gBestPosition[l] - R[p][l]));
			...
			System.out.println("iteration: " + j + " || Best: " + gBestValue);
		}
	}

	// fitness function calls functions imported from K-means class to find
	...
	public double calculateDistance(double[][] R, int clusterNum, double[][] clusterCenters)
	{
		...
		// uses averages to compare the distances between the centers and the
		// data
		// this method was implemented as there didn't seem to be a best option
		// - based
	...
	public void getClosest(ArrayList<Double> cenList, ArrayList<Double> dataList)
	{
		...
		{
			closest = 10000; // set to arbitrarily high number to be overwritten
								// immediately
